...
test('BF-TE-01: พนักงานกรอกเวลาทำงานปกติ 8 ชม. แล้ว Save Draft', async ({ page }) => {
  ...
  // ✅ Daily Total ต้องแสดงค่า 8.00
  const dailyTotalRow = page.locator('tr, div').filter({ hasText: /Daily Total/i }).first();
  await expect(dailyTotalRow).toBeVisible();
  await expect(dailyTotalRow).toContainText('8.00');
...
test('BF-TE-02: พนักงานบันทึก Overtime เกิน 8 ชม./วัน — ระบบแสดง Variance', async ({ page }) => {
  ...
  // Step 4: ตรวจ Variance row — ต้องมีค่าติดลบ (overtime = เกิน 8 ชม.)
  const varianceRow = page.locator('tr, div').filter({ hasText: /Variance/i }).first();
  await expect(varianceRow).toBeVisible();
  await expect(varianceRow).toContainText('-');
...
test('BF-TE-03: Submit Timesheet กรอกไม่ครบ 8 ชม. — ระบบ validate', async ({ page }) => {
  ...
  // ✅ ระบบ validate: ต้องมี warning dialog เรื่อง minimum hours หรือ status ยังเป็น Draft (submit ถูก block)
  const minHoursWarning = page.getByText(/minimum|ชั่วโมง|ไม่ครบ|hours.*required|incomplete/i).first();
  const warningDialog = page.locator('[role="dialog"], [role="alertdialog"]').first();
  const stillDraft = page.locator('[data-slot="badge"]:has-text("Draft")');

  const hasWarning = await minHoursWarning.isVisible({ timeout: 5000 }).catch(() => false);
  const hasWarningDialog = await warningDialog.isVisible({ timeout: 2000 }).catch(() => false);
  const isStillDraft = await stillDraft.isVisible({ timeout: 2000 }).catch(() => false);

  expect(hasWarning || hasWarningDialog || isStillDraft,
    'Submit with incomplete hours should show min-hours warning or remain Draft').toBe(true);
